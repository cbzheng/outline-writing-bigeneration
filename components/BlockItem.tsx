...
export const BlockItem: React.FC<BlockItemProps> = ({
  ...
  return (
    <div 
      className={`group relative flex flex-col gap-2 p-3 mb-2 rounded-md transition-all duration-200 border ${
        isSelected ? 'border-blue-400 ring-1 ring-blue-400 shadow-md z-10' : 'border-slate-200 hover:border-slate-300'
      } ${getLevelColor(block.level)}`}
      onClick={() => onSelect(block.id)}
    >
      <div className="flex items-start gap-2">
        {/* Drag Handle Placeholder */}
        <div className="mt-1 text-slate-400 cursor-grab active:cursor-grabbing" title="Drag to reorder (concept)">
        ...
        <div className="flex-1">
            <input
                ...
                className={`w-full bg-transparent outline-none ${
                    block.level === 0 ? 'font-bold text-slate-800' : 
